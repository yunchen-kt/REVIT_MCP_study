...
def main():
    ...
    schema = AnalysisResultSchema("Floor Altitude Heatmap", "Shows elevation of floor faces in meters")
    schemaIndex = -1
    
    results = sfm.GetRegisteredResults()
    for r in results:
        s = sfm.GetResultSchema(r)
        if s.Name == "Floor Altitude Heatmap":
            schemaIndex = r
            break
    ...
    with revit.Transaction("Analyze Floor Slope Heatmap"):
                                    ...
                                    if mesh:
                                        ...
                                        for pt in mesh.Vertices:
                                            ...
                                            if inter_res:
                                                ...
                                                elevation_m = pt.Z * 0.3048
                                                val_list = List[System.Double]()
                                                val_list.Add(elevation_m)
        ...
        if processed_faces > 0:
            forms.alert("Analysis complete! Processed {} faces. Please switch Analysis Display Style to view the heatmap (Meters).".format(processed_faces), title="Success")
        else:
            forms.alert("Could not find any top faces to analyze.", title="Warning")
