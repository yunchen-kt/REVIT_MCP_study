...
| MCP Tools | 共用 91 個工具 | 共用 91 個工具 | 共用 91 個工具 |
...
| `MCP/Core/CommandExecutor.cs` | Central command dispatcher (91+ commands), largest file |
...
| `MCP-Server/src/tools/` | Tool definitions (91 tools, 分 16 個模組) |
...
### 為什麼
MCP Server 已封裝 91 個 tools，處理了格式轉換、錯誤處理、重連機制。自寫腳本會：
- 繞過既有的錯誤處理與格式驗證
...
## Domain Knowledge & Workflow Files（42 個）
...
| 剖面基準線, section datum, 網格線調整, 樓層線延伸, Crop Box, 出圖排版 | `domain/section-datum-adjustment.md` |
| 剖面命名, 剖面編號, section numbering, 剖面自動編排, rename view, 視圖重新命名 | `domain/section-auto-numbering.md` |
